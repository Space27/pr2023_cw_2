...
int main(int argc, char **argv)
{
	...
	const char *opts = "f:s:t:w:c:C:T:S:o:FIrlbdHih";
	...
	while (opt != -1)
	{
		...
		if (condition == 1)
		{
			...
			else if (config.drawSqrPix)
			{
				config.drawSqrPix = 0;
				drawSquareNextPixels(config.srcColor, config.dstColor, config.width, img);
			}
		}
		else if (condition == -1)
			break;
